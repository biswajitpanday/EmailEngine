  ...
  implements IUserRepository
{
   ...
   */
  public async findByEmail(query: any): Promise<UserModel | null> {
    const body = {
      query: {
        match: query,
      },
    ...
    if (results.length > 0) {
      logger.info(`User found with email: ${query.email}`);
      return results[0];
    }

    logger.warn(`No user found with email: ${query.email}`);
    return null;
  }
}
